...
export default function HomeScreen() {
  const activeIndex = 1; // "National" looks active in your reference
    ...
    >
      ...
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          <Text style={styles.welcome}>
            Welcome, Amna <Text style={{ fontSize: 16 }}>👋</Text>
          </Text>
          <Text style={styles.subtitle}>Driving efficiency in every shipment</Text>

          {/* tiny route hint under the subtitle (dashed line look) */}
        ...
        <View style={styles.profileWrapper}>
          <Image
            source={{ uri: 'https://images.unsplash.com/photo-1544723795-3fb6469f5b39?q=80&w=200&auto=format&fit=crop' }}
      ...
      <View style={styles.grid}>
        {[
          ...
          <TouchableOpacity key={item.name} onPress={()=>{navigation.navigate('placeOrder')}} style={styles.card} activeOpacity={0.9}>
            {/* Left block: title + truck image near bottom-left */}
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitleTop}>{item.name.split(' ')[0]}</Text>
              <Text style={styles.cardTitleBottom}>
                {item.name.split(' ')[1] || ''}
              </Text>
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#fff', // warm off-white like the shot
  ...
  /* tabs */
  tabsWrap: { marginTop: 16, marginBottom: 14 },
  tabs: {
  ...
  activeTab: {
    backgroundColor: '#DAAE58', // light honey
    shadowColor: '#000',
    shadowOpacity: 0.08,
    shadowRadius: 6,
  ...
  card: {
    ...
    // make inner layout like the reference (info left, arrow button right)
    flexDirection: 'row',
  },
  cardTitleTop: {
    fontSize: 14,
